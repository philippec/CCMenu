...
- (void)testRetrievesEmptyListFromNonExistentDefaults
{
	...
	STAssertEquals((NSUInteger)0, [entries count], @"Should have returned empty list.");
	_verify(defaultsMock);
}
...
- (void)testRetrievesProjectListFromDefaults
{
	...
	STAssertEquals((NSUInteger)1, [entries count], @"Should have returned one project.");
...
- (void)testReturnsServerUrlHistory
{
	...
	STAssertEquals((NSUInteger)1, [history count], @"Should have returned correct list.");
